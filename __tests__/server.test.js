...
describe('api server courses', async () => {
	...
	it('should respond for post', async () => {
		...
		const res5 = await mockRequest.get(`/user/bayan3/courses`).set('authorization', `bearer ${res2.body.token}`);
		...
		let res8 = await mockRequest
			...
			.set('authorization', `bearer ${res2.body.token}`);
		let res9 = await mockRequest.get(`/user/bayan3/courses`).set('authorization', `bearer ${res2.body.token}`);
