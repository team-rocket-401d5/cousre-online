...
async function getCoursesHandler(req, res) {
	...
	const result = await directCourseModel.find({ user: resultedUser._id }).populate('user', 'username -_id');
	res.status(200).json(result);
}

async function publishCourseHandler(req, res) {
	createPublicCourse(req.params).then((result) => res.status(302).json(result)).catch((error) => {
